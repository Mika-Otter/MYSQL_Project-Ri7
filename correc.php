<?php
    ...
    try{
        ...
        //On définit le retour de nos fetchAll sur un tableau associatif
        $db->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
        //On sauvegarde notre base de donnée au cas où il y aurait une erreur
        $db->beginTransaction();
        // Si l'utilisateur clique sur le bouton de suppression
        if(isset($_POST["delete"])){
            $deleteSQL = $db -> prepare("DELETE FROM user WHERE id=:id");
            $deleteSQL->execute([":id"=>$_POST["delete"]]);
            $prompt=["success"=>"Utilisateur bien supprimé"];
        } 
        // Si l'utilisateur clique sur le bouton de confirmation, on lance la fonction updateUser()
        elseif(isset($_POST["confirm"])){
            $prompt = updateUser($db);
        }
        // Si l'utilisateur clique sur le bouton de création, on lance la fonction createUser()
...
<html>
    ...
    <body>
        ...
        <form method="post" id="addUser">
            ...
            <button type="submit" id="confirm" name="create">Ajouter</button>
        </form>
        <main>
            <table>
                <tr id="tableHeader">
                ...
                foreach($tableauRequete as $entry){ ?>
                    <tr class="<?= $confirm == $entry["id"] ? 'errorLine' : '' ?>">
                        <form method="post">
                            <?php 
...
<?php 
    function updateUser($db){
        ...
        // Si message d'erreur, on le return sans exécuter notre requête sql
